...
const Feeds = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // const response = await fetch("https://dummyjson.com/c/b2fd-a7b5-4943-a441");
      const response = await fetch("http://localhost:8080/api/posts");
  ...
  const renderBackend = () => {
    return backendData.map((eachPost) => (
      <Post userDetails={eachPost} key={eachPost.id}/>
    ));

    // {backendData.map((eachItem) => (
    //   <Post userDetails={eachItem} key={eachItem.id}/>
    // ))}
  }
